    ...
    <BasicTable>
      ...
      <template #status="{ row }">
        <ApiSwitch :value="row.status === EnableStatus.Enable" :api="(val) => handleChangeStatus(val, row)"
          :disabled="!hasAccessByCodes(['knowledge:dataset:edit'])" @reload="() => tableApi.query()" />
      </template>
      <template #toolbar-tools>
        <Space>
          <a-button :disabled="!vxeCheckboxChecked(tableApi)" danger type="primary"
            v-access:code="['knowledge:dataset:remove']" @click="handleMultiDelete">
            {{ $t("pages.common.delete") }}
          </a-button>
          <a-button type="primary" v-access:code="['knowledge:dataset:add']" @click="handleAdd">
            {{ $t("pages.common.add") }}
          </a-button>
        </Space>
      </template>
      <template #action="{ row }">
        <Space>
          <action-button v-access:code="['knowledge:dataset:edit']" @click.stop="handleEdit(row)">
            {{ $t("pages.common.edit") }}
          </action-button>
          <Popconfirm :get-popup-container="getPopupContainer" placement="left" title="确认删除？"
            @confirm="handleDelete(row)">
            <action-button danger v-access:code="['knowledge:dataset:remove']" @click.stop="">
              {{ $t("pages.common.delete") }}
            </action-button>
          </Popconfirm>
        </Space>
